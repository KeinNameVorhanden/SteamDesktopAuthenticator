...
namespace Steam_Desktop_Authenticator
{
    public partial class MainForm : Form
    {
        ...
        private async void btnTradeConfirmations_Click(object sender, EventArgs e)
        {
            ...
            btnTradeConfirmations.Text = "Trade Confirmations...";
        ...
        private void btnManageEncryption_Click(object sender, EventArgs e)
        {
            if (manifest.Encrypted)
            {
                ...
                {
                    MessageBox.Show("Passkeys dont match.");
                    return;
                }
        ...
        private void menuDeactivateAuthenticator_Click(object sender, EventArgs e)
        {
            ...
            if (scheme != 0)
            {
                ...
                {
                    MessageBox.Show("Confirmation codes dont match. Steam Guard not removed.");
                    return;
                }
                ...
                if (success)
                {
                    MessageBox.Show(String.Format("Steam Guard {0}. maFile will be deleted after hitting okay. If you need to make a backup, now's the time.", (scheme == 2 ? "removed completely" : "switched to emails")));                    this.manifest.RemoveAccount(currentAccount);
                    this.loadAccountsList();
                }
                else
                {
        ...
        private async void menuRefreshSession_Click(object sender, EventArgs e)
        {
            ...
            MessageBox.Show("Function disabled. Use \"Login again\".", "Refresh session", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
            ...
            try
            {
                ...
                foreach (var acc in accs)
                {
                    try
                    {
                        ...
                        {
                            if ((conf.conf_type == Confirmation.ConfirmationType.MarketSellTransaction && manifest.AutoConfirmMarketTransactions) ||
                                (conf.conf_type == Confirmation.ConfirmationType.Trade && manifest.AutoConfirmTrades))
                            {
                    ...
                    {
                        lblStatus.Text = "Refreshing the session";
